...
def init_model(args):
    tokenizer = AutoTokenizer.from_pretrained('./model/')#初始化tokenizer
    # 直接从.pth文件中加载模型
    if args.load == 0:
...
def main():
    ...
    parser.add_argument('--lora_name', default='None', type=str)#lora模型的名字
    parser.add_argument('--out_dir', default='out', type=str)#模型输出路径
